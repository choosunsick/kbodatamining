#' Calculating batter's homerun
#'
#' This function calculates the homerun for the specified player.
#' This function has an option for yearly and monthly.
#' default: the annualy homerun is calculated
#' If only the monthly parameter is TRUE, the monthly homerun is calculated
#' If you put a year in the yearly parameter, it calculates the homerun for that year.
#' If you put a month in the monthly parameter, it calculates the corresponding monthly homerun
#' If you put a year and a month in two parameter, the homerun for the time period you put in is calculated.
...
#' @return Batter's homerun for the year, month, or period put in is calculated.
#' @examples
#' ## default
#' player_HR(hanhwa_batter_2018,"이용규")
#' ## yearly=FALSE, monthly=TRUE
#' player_HR(hanhwa_batter_2018,"이용규",yearly=FALSE,monthly=TRUE)
#' ## yearly=TRUE, monthly=TRUE #error case
#' ## player_HR(hanhwa_batter_2018,"이용규",yearly=TRUE,monthly=TRUE)
#' ## yearly=2018, monthly=FALSE
#' player_HR(hanhwa_batter_2018,"이용규",yearly=2018)
#' ## yearly= TRUE, monthly=05
#' player_HR(hanhwa_batter_2018,"이용규",yearly=TRUE,monthly="05")
#' ## yearly=2018,monthly=05
#' player_HR(hanhwa_batter_2018,"이용규",yearly=2018,monthly="05")
#' @export

player_HR <-  function(data,playername,yearly=TRUE,monthly=FALSE){
  ...
  if(yearly == TRUE & monthly==FALSE){
    ...
    for(i in unique(substr(data$date,1,4))){
      ...
      temp[colname] <- factor_count(inning_data,"13")
    }
    ...
    temp <- factor_count(inning_data,"13")
  }
