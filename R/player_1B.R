#' Calculating batter's single hit
#'
#' This function calculates the single hit for the specified player.
#' This function has an option for yearly and monthly.
#' default: the annualy single hit is calculated
#' If only the monthly parameter is TRUE, the monthly single hit is calculated
#' If you put a year in the yearly parameter, it calculates the single hit for that year.
#' If you put a month in the monthly parameter, it calculates the corresponding monthly single hit
#' If you put a year and a month in two parameter, the single hit for the time period you put in is calculated.
...
#' @return Batter's single hit for the year, month, or period put in is calculated.
#' @examples
#' ## default
#' player_1B(hanhwa_batter_2018,"이용규")
#' ## yearly=FALSE, monthly=TRUE
#' player_1B(hanhwa_batter_2018,"이용규",yearly=FALSE,monthly=TRUE)
#' ## yearly=TRUE, monthly=TRUE #error case
#' player_1B(hanhwa_batter_2018,"이용규",yearly=TRUE,monthly=TRUE)
#' ## yearly=2018, monthly=FALSE
#' player_1B(hanhwa_batter_2018,"이용규",yearly=2018)
#' ## yearly= TRUE, monthly=05
#' player_1B(hanhwa_batter_2018,"이용규",yearly=TRUE,monthly=05)
#' ## yearly=2018,monthly=05
#' player_1B(hanhwa_batter_2018,"이용규",yearly=2018,monthly=05)
#' @export

player_1B <-  function(data,playername,yearly=TRUE,monthly=FALSE){
  ...
  if(yearly == TRUE & monthly==FALSE){
    ...
    for(i in unique(substr(data$date,1,4))){
      ...
      temp[colname] <- factor_count(inning_data,"10")
    }
    ...
    temp <- factor_count(inning_data,"10")
  }
