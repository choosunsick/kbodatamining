...
team_batter_compare_plot <- function(data){
  temp <-data[data$g>=10,]
  avg_compare <- ggplot(temp,aes(temp$avg,.data$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    geom_point(size = 3, aes(color = temp$team))+scale_x_continuous(breaks = seq(0,0.4,0.05))+
  ...
  obp_compare <- ggplot(temp,aes(temp$obp,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    geom_point(size = 3, aes(color = temp$team))+scale_x_continuous(breaks = seq(0,0.5,0.1))+
    scale_colour_manual(values=c("blue", "red"))+ggtitle("Compare OBP")+xlab("OBP")+ylab("PlayerName")+
    labs(colour="Team Names")+theme(axis.text.y = element_text(angle = 0,hjust=0.5,vjust=0.5,size = 7))
  slg_compare <- ggplot(temp,aes(temp$slg,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    geom_point(size = 3, aes(color = temp$team))+scale_x_continuous(breaks = seq(0,0.7,0.15))+
    scale_colour_manual(values=c("blue", "red"))+ggtitle("Compare SLG")+xlab("SLG")+ylab("PlayerName")+
    labs(colour="Team Names")+theme(axis.text.y = element_text(angle = 0,hjust=0.5,vjust=0.5,size = 7))
  babip_compare <- ggplot(temp,aes(temp$babip,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    geom_point(size = 3, aes(color = temp$team))+scale_x_continuous(breaks = seq(0,0.7,0.15))+
