...
#' If you put a month in the monthly parameter, it calculates the corresponding month AVG.
...
#' @param monlthly The default value is FALSE. Possible values are a specific month or a TRUE value.
#' @return The batting average for the year, month, or period entered is calculated.
#' @examples
#' ## default
#' AVG(hanhwa_batter_2018,"이용규")
#' ## yearly=FALSE, monthly= TRUE
#' AVG(hanhwa_batter_2018,"이용규",yearly=FALSE, monthly=TRUE)
#' ## yearly= FALSE, monthly==TRUE
#' AVG(hanhwa_batter_2018,"이용규",monthly==TRUE)
#' ## yearly=2018, monthly==FALSE
#' AVG(hanhwa_batter_2018,"이용규",yearly=2018)
#' ## yearly=FALSE, monthly==05
#' AVG(hanhwa_batter_2018,"이용규",yearly=FALSE,monthly=05)
#' ## yearly=2018,monthly==05
#' AVG(hanhwa_batter_2018,"이용규",yearly=2018,monthly=05)
#' @export
avg_calculate <- function(data,playername,yearly=TRUE,monthly=FALSE){
  ...
  if(yearly == TRUE & monthly==FALSE){
  ...
  else if(yearly==FALSE & is.numeric(as.numeric(monthly))==TRUE){
    avg <- list()
    for(i in unique(substr(data$date,1,4))){
  ...
  else if(is.numeric(yearly)==TRUE & is.numeric(as.numeric(monthly))==TRUE){
    ...
    print("Reread the description of the yearly and monthly arguments and use the function")
  }
  return(avg)
}
