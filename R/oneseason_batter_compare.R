...
#' default: The 'yearly' parameter is 2018. ㅆhe 'monthly' parameter is NULL
...
oneseason_compare_batter <- function(data=kbodatamining::hanhwa_batter_2018,playername1,playername2,yearly=2018,monthly=NULL){
  ...
  stat_data <- filter(reshape_data,reshape_data$variable %in% colnames(data)[1:23])
  ...
  percent_data <- filter(reshape_data,reshape_data$variable %in% colnames(data)[24:28])
