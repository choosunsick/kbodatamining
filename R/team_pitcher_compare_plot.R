...
team_pitcher_compare_plot <- function(data){
  temp<-data[data$g >=3,]
  era_compare <- ggplot(temp,aes(temp$era,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    geom_point(size = 3, aes(color = temp$team))+scale_x_continuous(breaks = seq(0,50,10))+
  ...
  k_compare <- ggplot(temp,aes(temp$k,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    geom_point(size = 3, aes(color = temp$team))+scale_x_continuous(breaks = seq(0,200,25))+
    scale_colour_manual(values=c("blue", "red"))+ggtitle("Compare K")+xlab("K")+ylab("PlayerName")+
    theme(axis.text.y = element_text(angle = 15,hjust=0.5,vjust=0.5,size = 7))
  oba_compare <- ggplot(temp,aes(temp$oba,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
    ...
    theme(axis.text.y = element_text(angle = 15,hjust=0.5,vjust=0.5,size = 7))
  p_ip_compare <- ggplot(temp,aes(temp$p_ip,temp$name))+geom_segment(aes(yend = temp$name), xend = 0, color = "grey50")+
